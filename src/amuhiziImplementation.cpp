 ...
 *
 *   
 *   13 March 2021: added pick_and_place functionality
...
bool computeJointAngles(double x, double y, double z, double pitch_angle_d, double roll_angle_d, double joint_angles[]) {
	...
	/* \93Modeling and Simulation of 5 DOF educational robot arm\94, 2nd International Conference on Advanced Computer Control (ICACC), 2010.  */
...
bool setJointAngles(double joint_angles[]) {
    ...
    if (debug)
        printf("setJointAngles(): angles %4.2f %4.2f %4.2f %4.2f %4.2f\n", joint_angles[0], joint_angles[1],
                joint_angles[2],joint_angles[3],joint_angles[4]);

#ifdef ROS

	/* ROS version: PUT ROS PUBLISHER CODE HERE */

	/* copy joint angles to the globally-accessible structure so that they can be used when constucting the topic 
     * message in the grasp() function */
	...
	joints_values = {robotConfigurationData.current_joint_value[0], robotConfigurationData.current_joint_value[1], 
                     robotConfigurationData.current_joint_value[2], robotConfigurationData.current_joint_value[3],
                     robotConfigurationData.current_joint_value[4], robotConfigurationData.current_joint_value[5]};
    ...
    if (valid_pose) {
        ...
        if (debug)
            printf("setJointAngles(): servo positions %d %d %d %d %d \n", positions[0], positions[1],
...
int spawn_brick(std::string name, std::string color, double x, double y, double z, double phi) {
    ...
    {
        if (debug)
            ROS_INFO("Spawned brick [%s] of color [%s] at position (%.3f %.3f %.3f %.2f %.3f %.3f)",
                    srv.response.name.c_str(), color.c_str(), (x/1000.0), (y/1000.0), (z/1000.0), 0.0, 0.0, radians(phi));
